...
def connectToNodes(node_list, is_to_seed = False):
	...
	for node in node_list:
		...
		if not node['Connected']:
			...
			node['socket'] = s;
			node['Connected'] = True
			print("Connected to seed, ", (node['IP'], node['PORT']))
			connected_nodes.append(node)
			if not is_to_seed:
...
def learnAboutPeers():
	...
	peers_set = set()
	print("Learning about peers!!!")
	for node in connected_nodes:
		if node['IsSeed'] :
			node['socket'].send(b"REQUEST_PEER")
			data = node['socket'].recv(1024).decode('utf-8')
			dataLen = int(data.split("#")[0])
			data = data.split("#")[1]
			#print(dataLen, len(data))
			while(len(data) < dataLen):
				data = data + node['socket'].recv(1024).decode('utf-8')
